var invoice = {
  ...
  convertFormSubmitButtonsToNormal: function() {
    var button = '<input id="edit-button" type="button" name="op" value="'+ $('#edit-submit').val() +'" class="form-submit" />';
    $('#edit-submit').replaceWith(button);
    $('#edit-button').attr('id', 'edit-submit');

    var button = '<input id="edit-button" type="button" name="op" value="'+ $('#edit-delete').val() +'" class="form-submit" />';
    $('#edit-delete').replaceWith(button);
    $('#edit-button').attr('id', 'edit-delete');

    $('#node-form').prepend('<input id="edit-op" type="hidden" name="op" />');

    // Set form submit action
    $('#node-form .form-submit').click(function() {
      $('#edit-op').val($(this).val());
      $('#node-form').submit();
    });
  },
  ...
  getUrl: function(path) {
    var url = "http://"+ Drupal.settings['invoice']['host'] + Drupal.settings['basePath'];
